...
function FinishSelect({ label, points, value, onChange }: { label: string; points: number[]; value: number; onChange: (value: number) => void }) {
    return <label>{label}<select value={value} onChange={(event) => onChange(Number(event.target.value))}>{points.map((amount, index) => <option key={index} value={index}>{index < points.length - 1 ? `P${index + 1} — ${amount} pts` : "No points / DNF"}</option>)}</select></label>;
}
